...
const RowContainer = ({ flag, data, scrollValue }) => {
  ...
  return (
    <div
      ref={rowContainer}
      className={`w-full flex items-center my-12  bg-cardOverlay
      scroll-smooth gap-3 ${
        flag
    ...
    >
      {data && data.length > 0 ? (
        data.map((item) => (
          <div
            key={item.id}
            className="w-300 h-[225px] min-w-[300px] md:w-340 md:min-w-[340px] bg-gray-100 h-auth backdrop-blur-lg p-2 my-2 hover:drop-shadow-2xl
